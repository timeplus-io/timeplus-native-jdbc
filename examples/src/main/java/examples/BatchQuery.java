    ...
    public static void main(String[] args) throws Exception {
        try (Connection connection = DriverManager.getConnection("jdbc:timeplus://127.0.0.1:8463")) {
            try (Statement stmt = connection.createStatement()) {
                ...
                try (PreparedStatement pstmt = connection.prepareStatement("select count(*) from table(test_jdbc_example) where age>? and age<=?")) {
                    pstmt.setByte(1, (byte) 10);
                    pstmt.setByte(2, (byte) 30);
                    printCount(pstmt);
                }

                try (PreparedStatement pstmt = connection.prepareStatement("select count(*) from table(test_jdbc_example) where name=?")) {
